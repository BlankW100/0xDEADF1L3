"""Format definitions and builders for 0xDEADF1L3"""
...
_MP3_FRAME_HDR  = b"\xFF\xFB\x90\x64"
_MP3_FRAME_SIZE = 417
...
def generate_bytes(
    ...
    """Generate file bytes for the given format and parameters."""
    n = int(size_raw) if size_raw.isdigit() else 0
    if ext == "pdf":
        return _make_pdf(title, author, date, desc, max(1, n), target)
    elif ext == "docx":
        return _make_docx(title, author, date, desc, max(1, n), target)
    elif ext == "pptx":
        return _make_pptx(title, author, date, desc, max(1, n), target)
    elif ext == "xlsx":
        return _make_xlsx(title, author, date, desc, n or 500, target)
    elif ext == "png":
        w, h = (int(v) for v in size_raw.lower().split("x"))
        return _make_png(w, h, target)
    elif ext == "mp3":
        prefix = _make_mp3_prefix(target)
        remaining = max(0, target - len(prefix))
        return prefix + os.urandom(remaining)
    elif ext == "mp4":
        prefix = _make_mp4_prefix(target)
        remaining = max(0, target - len(prefix))
        return prefix + os.urandom(remaining)
    else:  # txt, md
